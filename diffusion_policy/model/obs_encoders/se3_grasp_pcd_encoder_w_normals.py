...
class SE3GraspPointCloudSuperEncoder(ModuleAttrMixin):
    ...
    def compute_normals(self, pcd, camera_view):
    ...
    def encode_obs(self, obs):
        ...
        for i in range(batch):
            # TODO - make sure it is added here in the right dimension!
            # now compute the normals manually - note, we assume the center of the workspace exactly in the middle, i.e., at all 0s
            vectors[i, :, :, 0] = torch.tensor(self.compute_normals(pcd[i, ...].cpu().numpy(), np.zeros(3))).to(device)
