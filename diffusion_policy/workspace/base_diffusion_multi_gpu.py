...
from omegaconf import OmegaConf
from diffusion_policy.workspace.gpu_utils import synchronize_between_processes, seed_worker
                ...
                for local_epoch_idx in gepoch:
                            ...
                            with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}",
                                ...
                                for batch_idx, batch in enumerate(tepoch):
                                    ...
                                    if (cfg.training.max_val_steps is not None) \
                                        ...
                                        break
                            if len(val_losses) > 0:
                                values = synchronize_between_processes({'val_loss': torch.tensor(val_losses)})
                    ...
                    if ((self.epoch + 1) % cfg.training.model_evaluation_every) == 0:
                        with torch.no_grad():
                            ...
                            values = synchronize_between_processes(values)
