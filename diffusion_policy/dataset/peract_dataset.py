...
def create_sample_indices(task_group : zarr.hierarchy.Group, n_obs):
    indices = []
    for demo_group in task_group.values():
...
def collate_samples(datum, use_pc, use_rgb, use_mask, apply_cameras, use_features):
    ...
    if use_features:
        sample['obs']['features'] = {}
        sample['obs']['features']['res1'] = np.stack([cameras[camera]['features']['res1'][obs_idxs[-1]] for camera in apply_cameras])
        sample['obs']['features']['res2'] = np.stack([cameras[camera]['features']['res2'][obs_idxs[-1]] for camera in apply_cameras])
...
class RLBenchNextBestPoseDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        indices = create_sample_indices(task_group, n_obs_steps)
    ...
    def get_validation_dataset(self):
        val_set = RLBenchNextBestPoseDataset(
