...
def sample_random_se3(batch_size, device='cpu', dtype=torch.float32):
    lR = torch.randn((batch_size, 3), device=device, dtype=dtype)
    R = SO3_exp_map(lR)
    t = torch.randn((batch_size, 3), device=device, dtype=dtype)
