...
class URSAFlow(BaseImagePolicy):
...
class DiffusionHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.rotation_self_attn = URSATransformer(
            d_model=embedding_dim, nhead=num_attn_heads, num_layers=2
        )
        ...
        self.position_self_attn = URSATransformer(
            d_model=embedding_dim, nhead=num_attn_heads, num_layers=2
        )        
    ...
    def prediction_head(self,
        ...
        # cross-attention trajectory to full pcd
        query = {'centers': trajectory_pose[:, :, :3, 3], 'vectors': trajectory_pose[:, :, :3, :3]}
        key = {'centers': pcd, 'vectors': torch.zeros(pcd.shape + (3,)).to(pcd.device)}
        trajectory_features = self.cross_attn(
            tgt=trajectory_features, memory=pcd_features,
        ...
        vectors = torch.cat([curr_gripper_pose[:, :, :3, :3], trajectory_pose[:, :, :3, :3], torch.zeros(fps_pcd.shape + (3,)).to(fps_pcd.device)], 1)
        query = {'centers': centers, 'vectors': vectors}
        features = self.self_attn(tgt=query_features, memory=query_features, geometric_args={'query': query, 'key': query})

        num_trajectory = trajectory_pose.size(1)

        # Rotation head
        rotation = self.predict_rot(
            features, query, num_trajectory
        )
    ...
    def predict_pos(self, features, query, num_trajectory):
        position_features = self.position_self_attn(
            tgt=features, memory=features, geometric_args={'query': query, 'key': query}
        )        
    ...
    def predict_rot(self, features, query, num_trajectory):
