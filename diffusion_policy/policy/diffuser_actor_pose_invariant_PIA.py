...
from diffusion_policy.model.obs_encoders.diffuser_actor_pose_invariant_encoder_PIA import DiffuserActorEncoder
...
class DiffusionHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.rotation_self_attn = GeometryInvariantTransformer(
            dim=embedding_dim,
            dim_head=head_dim,
            mlp_dim=embedding_dim,
            heads=num_attn_heads,
        ...
        self.position_self_attn = GeometryInvariantTransformer(
            dim=embedding_dim,
            dim_head=head_dim,
            mlp_dim=embedding_dim,
            heads=num_attn_heads,
    ...
    def prediction_head(self,
        ...
        context = torch.cat([curr_gripper_pose[..., :3, 3], pcd], 1)
        features = torch.cat([curr_gripper_features, pcd_features], 1)
        ...
        # Self-attention
        poses = torch.cat([trajectory_pose, curr_gripper_pose], 1)
        features = torch.cat([trajectory_features, curr_gripper_features], 1)
        extras = {'x_poses': poses, 'x_types':'se3'}
        features = self.self_attn(x=features, extras=extras, diff_ts=time_embs)

        num_trajectory = trajectory_features.shape[1]

        # Rotation head
        rotation = self.predict_rot(
            features, poses, num_trajectory, time_embs
        )
    ...
    def predict_pos(self, features, poses, num_trajectory, time_embs):
        extras = {'x_poses': poses, 'x_types': 'se3'}
        position_features = self.position_self_attn(x=features, extras=extras, diff_ts=time_embs)
        position_features = position_features[:, :num_trajectory]  # (B, N, C)
        position_features = self.position_proj(position_features)  # (B, N, C)
        position = self.position_predictor(position_features)
        return position, position_features

    def predict_rot(self, features, poses, num_trajectory, time_embs):
        extras = {'x_poses': poses, 'x_types': 'se3'}
        rotation_features = self.rotation_self_attn(x=features, extras=extras, diff_ts=time_embs)
        rotation_features = rotation_features[:, :num_trajectory]  # (B, N, C)
