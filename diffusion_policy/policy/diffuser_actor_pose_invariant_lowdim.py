...
class DiffuserActor(BaseImagePolicy):
    ...
    def forward(
        ...
        for layer_pred in pred:
            ...
            rot = layer_pred[..., 3:6]
            loss = (
                30 * F.l1_loss(trans, dp, reduction='mean')
                + 10 * F.l1_loss(rot, dr, reduction='mean')
            )
...
class DiffusionHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.self_attn = InvariantPointTransformer(
            embedding_dim, 3, num_attn_heads, head_dim, kv_dim=None, use_adaln=True,
        ...
        self.position_self_attn = InvariantPointTransformer(
            embedding_dim, 2, num_attn_heads, head_dim, kv_dim=None,
            dropout=0.0, attention_module=InvariantPointAttention, point_dim=3)
    ...
    def prediction_head(self,
        ...
        extras = {'x_poses': gripper_pose, 'z_poses': context,
              'x_types':'se3', 'z_types':'3D'}

        # Cross attention from trajectory to pcd + curr_gripper
        gripper_features = self.cross_attn(x=gripper_features, z=features, extras=extras, diff_ts=time_embs)

        # Self attention among trajectory and curr_gripper
        features = torch.cat([gripper_features, curr_gripper_features], 1)
        poses = {}
        poses['rotations'] = torch.cat([gripper_pose[..., :3, :3], curr_gripper_pose[..., :3, :3]], 1)
        poses['translations'] = torch.cat([gripper_pose[..., :3, 3], curr_gripper_pose[..., :3, 3]], 1)
        features = self.self_attn(features, poses, diff_ts=time_embs)

        num_gripper = gripper_features.shape[1]

        # Rotation head
        rotation = self.predict_rot(
            features, poses, time_embs, num_gripper
        )
    ...
    def predict_pos(self, features, rel_pos, time_embs, num_gripper):
        position_features = self.position_self_attn(features, rel_pos, diff_ts=time_embs)
        position_features = position_features[:,:num_gripper]
        position_features = self.position_proj(position_features)  # (B, N, C)
        position = self.position_predictor(position_features)
        return position, position_features

    def predict_rot(self, features, poses, time_embs, num_gripper):
        rotation_features = self.rotation_self_attn(features, poses, diff_ts=time_embs)
