...
class LowdimObsEncoder(nn.Module):
    def __init__(self, n_obs_steps, action_dim, global_dim, nkeypoints):
        super().__init__()
        self.global_dim = global_dim
        self.nkeypoints = nkeypoints
        self.keypoint_embeddings = nn.Embedding(nkeypoints, global_dim)
        self.gripper_embeddings = nn.Embedding(n_obs_steps, global_dim)
        self.gripper_head = nn.Sequential(
            nn.Linear(action_dim, global_dim),
            nn.ReLU()
        )
        self.pcd_head = nn.Sequential(
    ...
    def forward(self, curr_gripper, pcd):
        B, _, _ = pcd.shape
        # B, L, D
        pcd_features = self.pcd_head(pcd) + self.keypoint_embeddings.weight.unsqueeze(0).repeat(B, 1, 1)
        pcd_features = torch.sum(pcd_features, dim=1, keepdim=True)
        # B, H, D
        gripper_features = self.gripper_head(curr_gripper) + self.gripper_embeddings.weight.unsqueeze(0).repeat(B, 1, 1)
        gripper_features = gripper_features + pcd_features
        global_features = self.out_projection(gripper_features)
...
class DiffusionUnetLowdimPolicy(BaseLowdimPolicy):
    ...
    def compute_loss(self, batch):
        ...
        # encode observation
        obs = self.obs_encoder(curr_gripper, pcd)
