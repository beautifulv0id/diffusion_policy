...
flags.DEFINE_bool('high_dim', False,
...
def run(i, lock, task_index, task_demo_indices, task_variation_count, results, file_lock, tasks):
    ...
    while True:
        ...
        with lock:
            ...
            if my_demo_index >= FLAGS.episodes_per_task:
                task_demo_indices[task_index.value] = my_demo_index = 0
                task_variation_count[task_index.value] += 1

            my_variation_count = task_variation_count[task_index.value]
        ...
        print('Process', i, '// Task:', task_env.get_name(),
                '// Variation:', my_variation_count, '// Demo:', my_demo_index)
        attempts = 10
        while attempts > 0:
            try:
                ...
                attempts -= 1
                if attempts > 0:
                    print('Process', i, 'failed collecting task', task_env.get_name(),
                          '(variation:', my_variation_count, ', example:', my_demo_index,
                          '). Retrying...')
                    continue
                problem = (
                    'Process %d failed collecting task %s (variation: %d, '
                    'example: %d). Skipping this task/variation.\n%s\n' % (
                        i, task_env.get_name(), my_variation_count, my_demo_index,
                        str(e))
                )
                print(problem)
                ...
                break
            episode_path = os.path.join(episodes_path, EPISODE_FOLDER % my_demo_index)
            with file_lock:
