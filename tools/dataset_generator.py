...
flags.DEFINE_bool('high_dim', True,
                    'Include high dimensional data in the dataset')
flags.DEFINE_bool('single_task_single_variation', True,
                  'Collect only one task and one variation')
...
def run(i, lock, task_index, variation_count, results, file_lock, tasks):
    ...
    obs_config.set_all_high_dim(FLAGS.high_dim)
    ...
    rlbench_env = RLBenchEnv(
        data_path=FLAGS.save_path,
        headless=True,
        collision_checking=False,
    )
    env = rlbench_env.env
    env._obs_config = obs_config
    rlbench_env.launch()
    ...
    while True:
        ...
        with lock:
            ...
            if task_index.value >= num_tasks:
                print('Task index value:', task_index.value)
        ...
        for ex_idx in range(FLAGS.episodes_per_task):
            ...
            while attempts > 0:
                ...
                valid = rlbench_env.verify_demo(demo, task_env, max_rrt_tries=1, demo_consistency_tries=10, verbose=False)
                if not valid:
                    attempts -= 1
                    print('Process %d failed verifying task %s (variation: %d, example: %d). Retrying.\n' % (
                        i, task_env.get_name(), my_variation_count, ex_idx))
                    if attempts > 0:
                        continue
                    problem = (
                        'Process %d failed verifying task %s (variation: %d, '
        ...
        for ex_idx in range(FLAGS.episodes_per_task):
            ...
            while attempts > 0:
                try:
                    ...
                    variation = 0 # TODO: remove this line
                    task_env = env.get_task(t)
...
def run_variation_for_task(i, lock, demo_index, results, file_lock, tasks_str):
    """Each thread will choose one task and variation, and then gather
    all the episodes_per_task for that variation."""
    task = task_file_to_task_class(tasks_str)
    variation = 0
    # Initialise each thread with random seed
    np.random.seed(None)
    ...
    while True:
        ...
        with lock:
            ...
            if demo_index.value >= FLAGS.episodes_per_task:
                print('Demo index value:', demo_index.value)
                print('Num demos:', FLAGS.episodes_per_task)
                print('Process', i, 'finished')
                break
        ...
        abort_variation = False
        print('Process', i, '// Task:', task_env.get_name(),
                '// Variation:', variation, '// Demo:', demo_index.value)
        attempts = 10
        while attempts > 0:
            try:
                task_env = env.get_task(task)
                ...
                problem = (
                    'Process %d failed collecting task %s (variation: %d, '
                    'example: %d). Skipping this task.\n%s\n' % (
                        i, task_env.get_name(), variation, demo_index.value,
                        str(e))
                )
                print(problem)
                tasks_with_problems += problem
            ...
            episode_path = os.path.join(episodes_path, EPISODE_FOLDER % demo_index.value)
...
def run_single_demo(demo_index, results, tasks_str):
    task = task_file_to_task_class(tasks_str)
    variation = 0
    # Initialise each thread with random seed
    np.random.seed(None)
    ...
    abort_variation = False
    print('Demo', demo_index, '// Task:', task_env.get_name(),
            '// Variation:', variation, '// Demo:', demo_index)
    attempts = 10
    while attempts > 0:
        try:
            task_env = env.get_task(task)
            ...
            problem = (
                'Process %d failed collecting task %s (variation: %d, '
                'example: %d). Skipping this task.\n%s\n' % (
                    demo_index, task_env.get_name(), variation, demo_index,
                    str(e))
            )
            print(problem)
            tasks_with_problems += problem
        ...
        episode_path = os.path.join(episodes_path, EPISODE_FOLDER % demo_index)
...
def main(argv):
    ...
    for ex_i in range(0, FLAGS.episodes_per_task, FLAGS.processes):
        processes = [Process(target=run_variation_for_task, args=(
            i, lock, demo_index, result_dict, file_lock, task_files[0]))
            for i in range(ex_i, min(FLAGS.episodes_per_task,ex_i+FLAGS.processes))]
