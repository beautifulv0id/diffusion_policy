        ...
        for task in gtask:
            ...
            with tqdm(range(num_demos), desc=f"Task: {task}",
                ...
                for demo_idx in tepoch:
                    demo_lowdim = get_stored_demos(amount = 1, variation_number=0, task_name=task, from_episode_number=demo_idx, image_paths=True, dataset_root=FLAGS.data_path, random_selection=False, obs_config=obs_config_low_dim)[0]
                    keypoints = _keypoint_discovery(demo_lowdim)
                    if 0 not in keypoints:
                        keypoints = [0] + keypoints
                    demo = get_stored_demos(amount = 1, variation_number=0, task_name=task, from_episode_number=demo_idx, image_paths=False, dataset_root=FLAGS.data_path, random_selection=False, obs_config=obs_config, obs_idxs=keypoints)[0]
